...
export const MovieDetail = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch(process.env.REACT_APP_BASE_URL + "recommend" + `?name=${movie_title}`);
    ...
    <main>
      ...
      <section className="max-w-7xl mx-auto py-7">
        {loading ?
          ...
          <>
            <h2 className="text-black text-2xl text-center dark:text-white">Recommend For You.</h2>
